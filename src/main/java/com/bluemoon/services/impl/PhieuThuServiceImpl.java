...
public class PhieuThuServiceImpl implements PhieuThuService {
    ...
    private static final String SELECT_ALL = 
            "SELECT pt.*, tk.hoTen as tenNguoiLap " +
            "FROM PhieuThu pt " +
            "LEFT JOIN TaiKhoan tk ON pt.maTaiKhoan = tk.id " +
            "JOIN DotThu dt ON pt.maDot = dt.id " +
            "WHERE dt.trangThai = 'Mo' OR pt.trangThai = 'DaThu' OR pt.trangThai = 'DaThanhToan' " +
            "ORDER BY pt.id DESC";
    ...
    private static final String SELECT_BY_MAHO = 
            ...
            "FROM PhieuThu pt " +
            "LEFT JOIN TaiKhoan tk ON pt.maTaiKhoan = tk.id " +
            "WHERE pt.maHo = ? ORDER BY pt.id";

    private static final String SELECT_BY_DOT = 
            "SELECT pt.*, tk.hoTen as tenNguoiLap " +
            "FROM PhieuThu pt " +
            "LEFT JOIN TaiKhoan tk ON pt.maTaiKhoan = tk.id " +
            "WHERE pt.maDot = ? ORDER BY pt.id";
    ...
    public int createPhieuThuWithDetails(PhieuThu phieuThu, List<ChiTietThu> chiTietList) {
        ...
        if (chiTietList != null && !chiTietList.isEmpty()) {
            ...
            for (ChiTietThu chiTiet : chiTietList) {
                ...
                // Commit transaction
                conn.commit();
                logger.log(Level.INFO, "Successfully created PhieuThu with details, id: " + maPhieu + " (tongTien will be auto-updated by trigger)");
        return maPhieu;
    ...
    private PhieuThu buildPhieuThuFromResultSet(ResultSet rs) throws SQLException {
        ...
        try {
            // Lấy cột 'tenNguoiLap' từ câu query SELECT
            String tenNguoiLap = rs.getString("tenNguoiLap");
            phieuThu.setTenNguoiLap(tenNguoiLap);
        } catch (SQLException e) {
            // Bỏ qua lỗi nếu câu query không có cột này (để tránh lỗi ở các hàm khác)
        }
                ...
                for (java.util.Map<String, Object> phieuData : phieuDataList) {
                    ...
                    if (maPhieu > 0) {
                        ...
                        for (ChiTietThu chiTiet : chiTietList) {
                            ...
                            // thanhTien là GENERATED column, database sẽ tự tính
    ...
    public Map<String, Object> getInvoiceDetail(int id) {
        ...
        String sql = "SELECT " +
                     ...
                     "LEFT JOIN NhanKhau nk ON hg.maChuHo = nk.soCCCD AND nk.hieuLuc = TRUE " +
