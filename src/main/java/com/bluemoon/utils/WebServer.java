...
public class WebServer {
    ...
    public void start() {
        ...
        app.post("/api/dot-thu", ctx -> {
            try {
                // 1. Đọc JSON thành Map để lấy dữ liệu thô
                @SuppressWarnings("unchecked")
                Map<String, Object> body = objectMapper.readValue(ctx.body(), Map.class);

                // 2. Lấy ID người tạo ra để dùng riêng (trước khi xóa)
                int userId = 1; // Mặc định là Admin
                if (body.containsKey("nguoiTaoId") && body.get("nguoiTaoId") != null) {
                    try {
                        userId = Integer.parseInt(body.get("nguoiTaoId").toString());
                    } catch (NumberFormatException e) {
                        userId = 1;
                    }
                }

                // XÓA TRƯỜNG "nguoiTaoId" KHỎI MAP 
                // Jackson không bị lỗi "Unrecognized field" khi convert sang DotThu
                body.remove("nguoiTaoId"); 

                // Chuyển đổi sang DotThu an toàn
                DotThu dotThu = objectMapper.convertValue(body, DotThu.class);

                // 4. Gọi hàm tạo đợt thu + phiếu thu
                int newId = dotThuService.createDotThuWithReceipts(dotThu, userId);

                if (newId != -1) {
                    dotThu.setId(newId);
                    ctx.status(201).json(createSuccessResponse("Tạo đợt thu và sinh phiếu thu thành công", dotThu));
                } else {
                    ctx.status(400).json(createErrorResponse("Lỗi khi tạo đợt thu (Database trả về lỗi)"));
                }
            } catch (Exception e) {
                handleException(ctx, e);
            }
        ...
        app.get("/api/phong", ctx -> {
            try {
                ...
                String sql = "SELECT p.soPhong, p.dienTich, p.giaTien, p.trangThai as trangThaiPhong, p.ghiChu, " +
                            ...
                            "FROM Phong p " +
                            "LEFT JOIN HoGiaDinh h ON p.soPhong = h.soPhong AND h.thoiGianKetThucO IS NULL " + 
                            "ORDER BY p.soPhong";
        ...
        app.get("/api/nhan-khau/lich-su/all", ctx -> {
            try {
                ctx.json(nhanKhauService.getAllHistory());
            } catch (Exception e) {
                handleException(ctx, e);
            }
        });
        app.post("/api/nhan-khau", ctx -> {
            try {
                ...
                // Log để debug
                logger.info("Creating resident: " + nhanKhau.getHoTen() + ", maHo: " + nhanKhau.getMaHo() + 
                           ", soCCCD: " + nhanKhau.getSoCCCD());
                ...
                if (success) {
                    ...
                    logger.warning("Failed to create resident: " + nhanKhau.getHoTen());
                    ctx.status(400).json(createErrorResponse("Failed to create resident. Please check the logs for details."));
                }
            } catch (Exception e) {
                logger.severe("Exception when creating resident: " + e.getMessage());
                e.printStackTrace();
                handleException(ctx, e);
            }
        });
